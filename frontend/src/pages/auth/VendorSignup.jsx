...
const VendorSignup = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // Success - use AuthContext signup which handles authentication
      const result = await signupUser(formData.email, formData.password, formData.name, 'vendor');
      
      if (!result.success) {
        throw new Error(result.error || 'Vendor registration failed');
      }
